...
<cite>
**本文档引用的文件**
- [ImproviserMcpTools.cs](file://ext\NPCLife\src\NPCLife\Workspace\ImproviserMcpTools.cs)
...
## 简介
本文件面向"即兴编剧代理系统"，系统性阐述即兴编剧（临时任务代理）在 RimLife 中的职责边界、触发机制、响应策略与协作方式。即兴编剧代理专注于处理突发性、独立性的日常事件，不维护跨轮次的剧情上下文，通过 MCP 工具进行即时叙事输出，并在必要时将事件路由至导演或编剧工作空间。

## 项目结构
围绕即兴编剧代理的关键模块包括：
- 工作空间与工具层：工作空间管理器、工作空间实现、三类 MCP 工具提供者（导演、编剧、即兴编剧）
...
IMP["ImproviserMcpProvider<br/>即兴编剧MCP工具"]
...
- [ImproviserMcpTools.cs:21-274](file://ext\NPCLife\src\NPCLife\Workspace\ImproviserMcpTools.cs#L21-L274)
...
## 核心组件
- 即兴编剧 MCP 工具提供者：提供工作空间查询、逐句台词推送、结束本轮、事件路由等工具，专用于日常对话与突发事件的即时叙事输出。
...
**章节来源**
- [ImproviserMcpTools.cs:36-45](file://ext\NPCLife\src\NPCLife\Workspace\ImproviserMcpTools.cs#L36-L45)
...
LLM-->>Agent : "响应(含工具调用)"
Agent->>IMP : "invoke(push_line/finish_round/route_events)"
IMP->>WS : "PushLine/FinishRound/RouteEvents"
WS->>WM : "更新状态/事件路由"
WM-->>WS : "持久化/状态同步"
...
**图表来源**
- [AgentLoop.cs:174-340](file://ext\NPCLife\src\NPCLife\Agent\AgentLoop.cs#L174-L340)
- [ImproviserMcpTools.cs:87-154](file://ext\NPCLife\src\NPCLife\Workspace\ImproviserMcpTools.cs#L87-L154)
- [WorkspaceImpl.cs:85-184](file://ext\NPCLife\src\NPCLife\Workspace\WorkspaceImpl.cs#L85-L184)
...
- 角色约束：仅允许即兴编剧身份调用推送与结束轮次。

```mermaid
classDiagram
class ImproviserMcpProvider {
+HookId : string
+HookName : string
...
ImproviserMcpProvider --> McpTool : "生成工具定义"
ImproviserMcpProvider --> WorkspaceImpl : "调用推送/结束/路由"
```
...
**章节来源**
- [ImproviserMcpTools.cs:36-45](file://ext\NPCLife\src\NPCLife\Workspace\ImproviserMcpTools.cs#L36-L45)
- [ImproviserMcpTools.cs:55-75](file://ext\NPCLife\src\NPCLife\Workspace\ImproviserMcpTools.cs#L55-L75)
- [ImproviserMcpTools.cs:87-115](file://ext\NPCLife\src\NPCLife\Workspace\ImproviserMcpTools.cs#L87-L115)
- [ImproviserMcpTools.cs:127-154](file://ext\NPCLife\src\NPCLife\Workspace\ImproviserMcpTools.cs#L127-L154)
- [ImproviserMcpTools.cs:165-232](file://ext\NPCLife\src\NPCLife\Workspace\ImproviserMcpTools.cs#L165-L232)
...
## 依赖关系分析
- 组件耦合：
  - 即兴编剧 MCP 工具依赖工作空间接口与日志接口，通过注入获取 WorkspaceManager。
...
**图表来源**
- [ImproviserMcpTools.cs:23-30](file://ext\NPCLife\src\NPCLife\Workspace\ImproviserMcpTools.cs#L23-L30)
...
## 故障排除指南
- 工具调用失败
  - 检查调用者角色与工作空间状态：仅允许即兴编剧/编剧身份调用推送与结束轮次，且工作空间需处于 Active 状态。
...
## 结论
即兴编剧代理系统通过明确的角色分工与严格的工具调用约束，实现了对突发性与独立性事件的高效处理。其与导演、编剧工作空间的协作机制确保了叙事的一致性与扩展性。结合 MCP 框架的工具定义与调用能力，系统在响应速度、对话质量与情感智能方面具备良好的可配置性与可扩展性。

## 附录

### 即兴编剧代理的日常行为模式配置示例（步骤说明）
- 创建工作空间
  - 使用即兴编剧工具获取工作空间摘要，确认事件池与标签状态。
  - 参考：[GetWorkspace:57-69](file://ext\NPCLife\src\NPCLife\Workspace\ImproviserMcpTools.cs#L57-L69)
- 推送台词
  - 逐句调用 push_line，设置说话者、文本、延迟与类型；支持多句并行调用。
  - 参考：[PushLine:87-115](file://ext\NPCLife\src\NPCLife\Workspace\ImproviserMcpTools.cs#L87-L115)
- 结束本轮
  - 在推送完所有台词后调用 finish_round，提供本轮摘要与触发事件列表。
  - 参考：[FinishRound:127-154](file://ext\NPCLife\src\NPCLife\Workspace\ImproviserMcpTools.cs#L127-L154)
- 事件路由
  - 将不适合当前线的事件路由至导演或编剧工作空间，可附加留言与关键词。
  - 参考：[RouteEvents:165-232](file://ext\NPCLife\src\NPCLife\Workspace\ImproviserMcpTools.cs#L165-L232)

### 即兴编剧代理提示词要点
- 职责边界：处理突发性、独立性事件，不维护剧情延续性。
- 叙事风格：轻快、即兴、快速响应；每轮仅处理当前批次事件。
- 工具使用：严格遵循 push_line → finish_round 流程；必要时使用 route_events。
- 参考：[ImproviserPrompt.txt:1-18](file://ext\NPCLife\src\NPCLife\Prompts\ImproviserPrompt.txt#L1-L18)

### 系统提示词配置
- 即兴编剧系统提示词：通过 PromptConfig.DefaultImproviserPrompt 加载，支持动态添加即兴编剧附加指令。
- 提示词构建：在 RimLifeCore.BuildImproviserSystemPrompt 中构建，包含工作空间上下文、导演工作空间信息和格式规范。
- 参考：[PromptConfig.cs:27-50](file://ext\NPCLife\src\NPCLife\Driver\PromptConfig.cs#L27-L50)
- 参考：[RimLifeCore.cs:980-1001](file://Source\Infrastructure\RimLifeCore.cs#L980-L1001)