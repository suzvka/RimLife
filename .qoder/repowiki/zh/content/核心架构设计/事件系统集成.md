...
## 更新摘要
**所做更改**
- 新增了事件缓冲层（EventBuffer）的详细实现说明，包括去抖机制和批量推送策略
- 更新了事件拦截机制，反映EventBuffer在事件流中的关键作用
- 增强了性能考量章节，重点说明事件缓冲对系统性能的提升
- 更新了故障排查指南，增加EventBuffer相关的问题诊断
- 完善了架构图表，展示EventBuffer在事件处理流程中的位置
...
ECM["EventCardMapper"]
EBuf["EventBuffer<br/>事件缓冲层<br/>去抖/批量推送"]
end
...
- 事件拦截与映射
  ...
  - **EventBuffer：实现事件去抖和批量推送，通过 IdleTimeoutTicks 控制事件合并时机，显著减少事件风暴对系统的影响。**
- 事件池与路由
...
Note over Buffer : "IdleTimeoutTicks 去抖<br/>批量推送"
