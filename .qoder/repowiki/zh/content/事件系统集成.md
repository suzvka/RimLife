...
## 引言
本文件面向 RimWorld 事件系统集成，系统性阐述事件拦截机制、事件映射器实现、事件路由与阈值触发、定时器脉冲配置、与工作空间管理的集成方式，并提供流程图与排障建议。内容兼顾初学者易懂与资深开发者所需的技术深度。
...
- 配置：DriverConfig 控制各角色阈值、定时器间隔、历史容量与最大 Agent 轮次。
...
## 架构总览
事件从 RimWorld 游戏层产生，经由 Harmony 钩子进入映射器，生成 IGameEvent 事件卡，写入工作空间事件池。事件池根据 DriverConfig 的阈值触发 OnThresholdReached，进而激活对应角色的 Agent 循环。同时，RimWorldAgentDriver 驱动全局时钟，按角色定时器间隔注入 TimerPulse 事件，确保系统不会因事件稀少而停滞。
...
- LetterStack.ReceiveLetter：在 Postfix 中调用 EventCardMapper.FromLetter，将 LetterDef、标签、正文、LookTargets、派系等映射为事件卡，并写入工作空间事件池。
...
- 外部依赖
  - Harmony（事件钩子）
  - Verse/RimWorld（游戏对象）
  - NPCLife 框架（EventBus、Workspace、DriverConfig、Cards）

```mermaid
graph LR
EH["EventHooks"] --> ECM["EventCardMapper"]
ECM --> WM["WorkspaceManager"]
...
## 结论
该事件系统通过 Harmony 钩子与映射器将 RimWorld 事件标准化为 IGameEvent，并借助 WorkspaceEventPool 的阈值与定时器脉冲机制，实现对多角色 Agent 的自动激活与路由。结合 EventBus 的事件总线与 WorkspaceManager 的工作空间管理，形成高内聚、低耦合的事件处理闭环。通过合理的阈值与定时器配置、异步封装与错误隔离，系统在保证实时性的同时具备良好的可维护性与扩展性。
