...
- 补充JsonWriter容量优化细节
- 添加事件缓冲诊断日志机制
- **新增事件颜色标签系统**：EventCardMapper新增GetLetterColorLabel方法，实现威胁级别、积极事件和信息消息的颜色编码系统，支持red/orangeRed/blue/white四种颜色标签
...
## 引言
本文件面向 RimWorld 事件系统集成，系统性阐述事件拦截机制、事件映射器实现、事件路由与阈值触发、定时器脉冲配置、与工作空间管理的集成方式，并提供流程图与排障建议。内容兼顾初学者易懂与资深开发者所需的技术深度。本次更新重点关注事件缓冲系统的性能优化、EventCardMapper的性能改进以及自检工具的增强功能，**特别新增了事件颜色标签系统**，为事件分类和可视化提供更直观的支持。
...
- **颜色标签系统**：EventCardMapper 新增 GetLetterColorLabel 方法，实现威胁级别、积极事件和信息消息的颜色编码系统，支持red/orangeRed/blue/white四种颜色标签。
...
- EventCardMapper
  ...
  - **颜色标签系统**：GetLetterColorLabel 将 LetterDef 映射到颜色标签，支持 red/orangeRed/blue/white 四种颜色，对齐 RimWorld 信封 UI 颜色。
...
- [EventCardMapper.cs:470-483](file://Source/Mappers/EventCardMapper.cs#L470-L483)
...
### 事件颜色标签系统 - **新增**
EventCardMapper 新增了 GetLetterColorLabel 方法，实现了完整的颜色编码系统，用于事件分类和可视化：

- **颜色映射规则**：
  - red：ThreatBig（大威胁）、Death（死亡）、BadUrgent（紧急负面）
  - orangeRed：ThreatSmall（小威胁）、NegativeEvent（负面事件）、Bad（负面）
  - blue：PositiveEvent（积极事件）、Good（正面）
  - white：默认/未知事件

- **与现有系统集成**：
  - 在 FromLetter 方法中自动添加到 Payload["letterColor"]
  - 与 MapLetterImportance 和 BuildLetterTags 协同工作
  - 支持 UI 层的颜色渲染和事件分类

- **对齐 RimWorld 信封 UI**：颜色标签完全对齐 RimWorld 原生信封颜色系统，便于用户理解和识别事件严重程度。

```mermaid
flowchart TD
Input["LetterDef"] --> CheckNull{"LetterDef 是否为空?"}
CheckNull --> |是| ReturnWhite["返回 'white'"]
CheckNull --> |否| CheckDef["检查 defName"]
CheckDef --> ThreatBig{"ThreatBig/Death/BadUrgent?"}
CheckDef --> ThreatSmall{"ThreatSmall/NegativeEvent/Bad?"}
CheckDef --> Positive{"PositiveEvent/Good?"}
CheckDef --> Default["其他情况"]
ThreatBig --> Red["返回 'red'"]
ThreatSmall --> OrangeRed["返回 'orangeRed'"]
Positive --> Blue["返回 'blue'"]
Default --> White["返回 'white'"]
ReturnWhite --> End["完成"]
Red --> End
OrangeRed --> End
Blue --> End
...
**章节来源**
- [EventCardMapper.cs:470-483](file://Source/Mappers/EventCardMapper.cs#L470-L483)
- [EventCardMapper.cs:275-277](file://Source/Mappers/EventCardMapper.cs#L275-L277)
...
- **诊断日志**：EventBuffer 添加详细的事件进入缓冲日志，便于性能监控和调试。
- **颜色标签缓存**：GetLetterColorLabel 方法使用简单字符串比较，避免复杂计算开销。
...
- **颜色标签显示异常**
  - 检查 GetLetterColorLabel 方法的 LetterDef 映射是否正确；确认 LetterDef.defName 是否符合预期。
  - 参考：[EventCardMapper.cs:470-483](file://Source/Mappers/EventCardMapper.cs#L470-L483)
- **交互历史缺失**
...
## 结论
该事件系统通过 Harmony 钩子与映射器将 RimWorld 事件标准化为 IGameEvent，并借助 WorkspaceEventPool 的阈值与定时器脉冲机制，实现对多角色 Agent 的自动激活与路由。新增的事件缓冲系统通过空闲超时优化（默认600，约10秒）实现了事件合并去抖，显著提升了系统性能。**新增的颜色标签系统进一步增强了事件的可视化和分类能力**，通过 red/orangeRed/blue/white 四种颜色标签清晰区分事件的严重程度和性质。结合 EventBus 的事件总线与 WorkspaceManager 的工作空间管理，形成高内聚、低耦合的事件处理闭环。通过合理的阈值与定时器配置、异步封装与错误隔离，以及 JsonWriter 的容量优化和自检工具的全面测试，系统在保证实时性的同时具备良好的可维护性与扩展性。
...
- 颜色标签系统实现：[EventCardMapper.cs:470-483](file://Source/Mappers/EventCardMapper.cs#L470-L483)