...
- [ImproviserPrompt.txt](file://ext\NPCLife\Prompts\ImproviserPrompt.txt)
...
## 更新摘要
**所做更改**
- 将所有"自由职业者"相关内容全面更新为"即兴编剧"
- 更新系统名称、配置项、UI 标签、提示词等所有相关术语
- 修改工作空间管理器中的相关注释和文档字符串
- 更新角色记忆系统集成部分的术语一致性
...
即兴编剧代理系统是 RimLife 项目中的一个关键组件，专门负责处理日常对话和临时性事件响应。该系统通过 MCP（Model Context Protocol）工具提供者实现，能够独立于主线剧情进行即时性的角色互动和对话生成。
...
**章节来源**
- [ImproviserPrompt.txt:1-18](file://ext\NPCLife\Prompts\ImproviserPrompt.txt#L1-L18)
- [RimLifeCore.cs:760-788](file://Source\Infrastructure\RimLifeCore.cs#L760-L788)

## 项目结构

即兴编剧代理系统主要分布在以下目录结构中：

```mermaid
graph TB
subgraph "NPCLife 核心模块"
A[Workspace] --> B[ImproviserMcpTools.cs]
...
**图表来源**
- [ImproviserMcpTools.cs:1-274](file://ext\NPCLife\src\NPCLife\Workspace\ImproviserMcpTools.cs#L1-L274)
...
即兴编剧 MCP 工具提供者是系统的核心组件，实现了 IMcpHookProvider 接口，提供以下四个主要工具：

1. **impr_get_workspace**: 获取工作空间轻量信息
...
即兴编剧代理系统采用分层架构设计，通过事件驱动的方式实现异步处理：

```mermaid
sequenceDiagram
participant Game as 游戏引擎
participant Pool as 事件池
participant Agent as 即兴编剧代理
...
即兴编剧 MCP 工具提供者实现了特定的工具集，专门为临时任务代理设计：
...
ImproviserMcpProvider ..|> IMcpHookProvider
```

**图表来源**
- [ImproviserMcpTools.cs:21-45](file://ext\NPCLife\src\NPCLife\Workspace\ImproviserMcpTools.cs#L21-L45)
...
即兴编剧代理系统与角色记忆系统的集成通过 PawnMemoryProvider 实现：
...
G[ImproviserMcpProvider] --> H[IWorkspaceManager]
...
**章节来源**
- [ImproviserMcpTools.cs:110-115](file://ext\NPCLife\src\NPCLife\Workspace\ImproviserMcpTools.cs#L110-L115)
...
即兴编剧代理系统通过精心设计的架构实现了高效、灵活的临时性事件处理机制。系统的主要优势包括：
...
**章节来源**
- [RimLifeCore.cs:750-949](file://Source\Infrastructure\RimLifeCore.cs#L750-L949)
