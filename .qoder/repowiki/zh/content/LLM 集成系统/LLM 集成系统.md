...
## 架构总览
系统采用“服务接口 + 适配器 + 访问器 + 凭证管理”的分层架构。UI 通过访问器发起异步调用，访问器根据凭证提供商类型选择对应适配器，适配器将统一请求转换为特定 API 的请求格式并发起 HTTP 调用，再将响应转换为统一响应格式返回给 UI。凭证注册表负责持久化与激活顺序管理，支持多凭证回退。
...
Validate --> |有效| Build["构建 OpenAI 请求 JSON"]
...
Filter --> Build["构建 Anthropic 请求 JSON"]
Build --> Send["发送 HTTP 请求"]
Send --> Resp{"响应成功?"}
Resp --> |否| HttpErr["抛出 HTTP 异常"]
Resp --> |是| Parse["解析响应 JSON"]
...
- 温度参数控制
  - LlmRequest 支持 Temperature 参数（0~2），null 表示使用 API 默认值。适配器在构建请求时按需写入温度字段。
...
## 结论
该 LLM 集成系统通过统一数据模型与适配器模式，实现了对 OpenAI 与 Anthropic 的一致接入，配合凭证注册表与访问器的多凭证回退机制，提供了稳定、可扩展且易维护的集成方案。系统在 UI 层提供直观的配置与提示词管理能力，并通过 Token 统计与连通性测试辅助成本控制与问题排查。对于初学者，可从 UI 页面与提示词模板入手；对于高级用户，可通过自定义适配器与凭证持久化进一步扩展。
...
- 凭证配置
  ...
  - 超时秒数：根据网络状况调整。
- 提示词模板
  - 使用预置模板（导演/自由人/编剧）作为起点，结合角色背景与任务目标微调。
- 多凭证回退
