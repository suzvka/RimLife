...
- 驱动配置（DriverConfig）
  - 定义各角色的事件数量阈值、重要度阈值、定时器脉冲间隔（秒）、历史缓冲容量与最大轮数等参数。
...
- 定时器脉冲
  - 导演与临时任务代理支持"定时器脉冲间隔（秒）"，0 表示禁用；剧情编剧不支持定时器。
  - 定时器脉冲会向事件池注入 TimerPulse 事件，驱动代理周期性处理。
  - **更新**：定时器间隔单位已从ticks改为seconds，与游戏速度无关。
- 通用配置
...
- 定时器脉冲机制
  - **更新**：定时器间隔单位已从ticks改为seconds，与游戏速度无关。
  - 每帧计算经过的游戏tick数（deltaTicks），根据当前游戏速度倍率换算为现实秒。
  - 使用积分算法：1现实秒 = 60 × speedMultiplier 个tick，确保1现实秒 = 1积分。
  - 导演和临时任务代理的定时器脉冲基于累积的现实秒数触发。
- 积分算法实现
  - 核心积分算法：scorePerTick = 1f / (60f × speedMult)，addedScore = deltaTicks × scorePerTick
  - 累积器：导演和临时任务代理分别维护独立的累积器（_directorAccumSec、_freelancerAccumSec）
  - 触发机制：当累积值达到阈值时注入TimerPulse事件，然后从累积值中扣除阈值。
- 速度无关特性
  - **新增**：定时器脉冲与游戏速度完全无关，确保在Normal、Fast、Superfast、Ultrafast等各种速度下表现一致。
  - 通过积分算法自动适应不同的游戏速度倍率。

**章节来源**
- [RimLifeCore.cs:546-587](file://Source/Infrastructure/RimLifeCore.cs#L546-L587)
- [RimLifeCore.cs:594-605](file://Source/Infrastructure/RimLifeCore.cs#L594-L605)
- [RimLifeCore.cs:642-657](file://Source/Infrastructure/RimLifeCore.cs#L642-L657)
...
- 定时器脉冲频率
  - **更新**：定时器间隔单位为秒，且与游戏速度无关，确保在各种速度下表现一致。
...
- 定时器无效
  - **更新**：确认定时器间隔大于 0；剧情编剧不支持定时器；检查角色是否为导演或临时任务代理。
  - **新增**：如果定时器在某些游戏速度下表现异常，请检查是否正确设置了秒数而非ticks。
- 性能异常
...
## 结论
"叙事配置页面"提供了对导演、临时任务代理与剧情编剧的精细化参数控制，配合事件阈值触发、定时器脉冲与处理频率限制，能够有效平衡叙事质量与系统性能。**更新**：定时器间隔现采用秒为单位且与游戏速度无关，确保在Normal、Fast、Superfast、Ultrafast等各种速度下表现一致。通过工作空间生命周期管理与事件路由规则，系统实现了稳定的叙事驱动与可控的资源消耗。建议以默认值为基础，结合实际游戏节奏与事件密度，逐步微调参数，以获得最佳叙事体验。
...
### 定时器脉冲配置最佳实践
- **时间单位选择**
  - 导演定时器：建议设置为 30-120 秒，平衡响应速度与资源消耗。
  - Freelancer 定时器：建议设置为 60-300 秒，给予临时任务更多思考时间。
- **速度无关性优势**
  - 在任何游戏速度下，定时器触发间隔保持一致，避免因速度变化导致的叙事节奏不稳定。
- **性能优化建议**
  - 高频事件场景：适当降低定时器间隔，但避免过短导致CPU占用过高。
  - 低频事件场景：适当提高定时器间隔，减少不必要的代理激活。
- **调试技巧**
  - 使用 GetTimerPulseAccumulator 方法监控定时器累积值，验证配置是否正确生效。
  - 通过 GetCurrentSpeedMultiplier 方法确认当前游戏速度倍率，排除速度相关问题。
