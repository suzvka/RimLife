...
namespace RimLife.Infrastructure.Llm
{
    ...
    {
        /// <summary>
        /// 创建 LlmAccessor 实例。无任何初始化参数——每个方法调用自包含。
        /// </summary>
        public LlmAccessor()
        {
        }
            ...
            LlmRequest request, IReadOnlyList<LlmCredential> credentials, CancellationToken ct)
        {
            ...
            Task.Run(async () =>
            {
                ...
                for (int i = 0; i < credentials.Count; i++)
                {
                    ...
                    // 还有下一个凭证则继续
                    RimLifeCore.Logger?.Warning(
                        $"[RimLife.LlmAccessor] Fallback: credential[{i}] ({credential.ModelName}) failed: {lastError}. " +
        ...
        /// </summary>
        internal static ILlmApiProvider CreateAdapter(LlmCredential credential)
        {
            if (credential == null)
